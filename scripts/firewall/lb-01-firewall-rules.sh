...
# Entrada: Permitir tráfego Web da Internet (HTTP e HTTPS)
...
#Permite que o lb-01 e o lb-02 (10.10.10.11) comuniquem para saber qual está ativo
sudo iptables -A INPUT -p vrrp -s 10.10.10.11 -j ACCEPT
sudo iptables -A OUTPUT -p vrrp -d 10.10.10.11 -j ACCEPT

# Saída: Enviar o tráfego Web para os Servidores Aplicacionais na porta 8000
sudo iptables -A OUTPUT -p tcp -m iprange --dst-range 10.10.10.20-10.10.10.29 --dport 8000 -j ACCEPT

# Saída: Permitir DNS e HTTPS para a API da Google Cloud (gcloud)
