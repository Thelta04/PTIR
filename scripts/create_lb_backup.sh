#!/bin/bash
# scripts/create_lb_backup.sh
# Creates and starts the backup load balancer (lb-02) in passive mode.
# Usage: ./create_lb_backup.sh
...
echo "Creating Load Balancer Backup VM ($LB_BACKUP_IP)..."
gcloud compute instances create "$INSTANCE" \
    ...
    --private-network-ip="$LB_BACKUP_IP" \
    --network="$NETWORK" \
    --subnet="$SUBNET" \
...
remote_scp "$INSTANCE" \
    ...
    "$SCRIPT_DIR/firewall/lb-02-firewall-rules.sh"

remote_exec "$INSTANCE" "
    set -e
    source /tmp/config.sh
    sudo mkdir -p \$TARGET_DIR/scripts
    sudo mv /tmp/setup_lb.sh /tmp/lb_healthcheck.sh /tmp/config.sh /tmp/utils.sh /tmp/check_nginx.sh /tmp/notify_master.sh /tmp/lb-02-firewall-rules.sh \$TARGET_DIR/scripts/
    sudo chmod +x \$TARGET_DIR/scripts/setup_lb.sh \$TARGET_DIR/scripts/lb_healthcheck.sh \$TARGET_DIR/scripts/check_nginx.sh \$TARGET_DIR/scripts/notify_master.sh \$TARGET_DIR/scripts/lb-02-firewall-rules.sh
    sudo \$TARGET_DIR/scripts/setup_lb.sh '$WEBAPP_IPS' '$LB_PRIMARY_IP'
    sudo \$TARGET_DIR/scripts/lb-02-firewall-rules.sh
" || { echo "ERROR: Failed to setup $INSTANCE"; exit 1; }

echo ""
echo "Backup load balancer '$INSTANCE' ($LB_BACKUP_IP) is running in passive (BACKUP) mode."
