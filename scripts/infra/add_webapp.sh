...
# Auto-deploy webapp
echo ""
echo "=== Deploying webapp to all instances ==="
#!/bin/bash
...
remote_exec "$NEW_IP" "
    ...
    /tmp/setup_webapp.sh '$TARGET_DIR' '$REMOTE_USER' '$FIRST_VM' '$NEW_NAME'
" || { echo "ERROR: Deployment FAILED on $NEW_NAME. Aborting rolling update!"; exit 1; }
...
echo "WEBAPP_IPS=$NEW_IP" # Useful for LB deployment if run manually
